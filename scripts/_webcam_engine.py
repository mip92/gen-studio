# -*- coding: utf-8 -*-
"""Shot engine for webcam («ТЫ — Вебкам-модель»). cell_shaded (Graphic_Novel_Illustration), neon digital glow.
Track B, 2nd person, f5 (female). §5.3a env->static. BATCH: nameless country, no currency, silent breathers ok.
CLEAN ON SCREEN: no nudity/explicit; the taboo is thematic (monetized intimacy). Clothed, screen-lit framing only.
seed_act(scene_key, palette, shots, render_mode='static', id_region='0')
  shots tuple: (code, ch, loc, subj, st, ang, mv, tod, broll, iconic, narr); ch=None=env | profileCode
"""
import json, psycopg2
PFX="7e650000-0000-4000-8000-"; PROJ=PFX+"000000000001"
ROUTE_C="webcam_character_ip"; ROUTE_E="webcam_environment"

STYLE=("cinematic graphic novel illustration, glossy neon comic book panel, bold clean black ink outline, "
 "cell-shaded coloring with vivid neon glow, saturated magenta and cyan rim light, digital screen-lit mood, "
 "16:9 cinematic composition, no photorealism, no 3D render, no plastic skin, modern illustrated comic aesthetic, ")
TECH=("clean bold ink outline on every edge, cell-shaded color blocks with neon rim light and glowing highlights, ")
ENV_MOT=("only ambient environmental motion, slow flicker of neon and screen glow, gently shifting coloured light and shadow, "
 "faint drifting haze, an empty still scene with no people and no figures, the illustration barely coming to life, hand-drawn animation cadence")
...
CHAR_MOT=("subtle natural motion of only the figure already present in the frame, small breathing and quiet micro-movements "
 "continuing the moment, no new people entering, no extra figures appearing, the illustration in motion, hand-drawn animation cadence")
...
def seed_act(scene_key, palette, shots, render_mode="static", id_region="0"):
    ...
    for idx,(code,ch,loc,subj,st,ang,mv,tod,broll,iconic,narr) in enumerate(shots):
        ...
        if ch is not None and ch not in prof: raise SystemExit("unknown profile %s on %s"%(ch,code))
    ...
    print("  %-12s shots=%2d scale3run=%s short=%s nodot=%s"%(scene_key,len(shots),(",".join(runs) if runs else "0"),(",".join("%s:%d"%(c,w) for c,w in short) if short else "0"),(",".join(nodot) if nodot else "0")))
