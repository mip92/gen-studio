# -*- coding: utf-8 -*-
"""Seed workflow_templates + workflow_routes for project `tiler`. Idempotent."""
import sys
import psycopg2
...
TEMPLATES = [
    ("a1", "char_ip_graphic_novel",     "tiler/comfy/scene_single_character_graphic_novel_api.json",
     "Single-character cell-shaded scene (IP-Adapter anchor)"),
    ("a2", "environment_graphic_novel", "tiler/comfy/scene_environment_graphic_novel_api.json",
     "Environment / B-roll cell-shaded scene (no character)"),
]
ROUTES = [
    ("b1", "tiler_character_ip"),
    ("b2", "tiler_environment"),
]
...
def main():
    ...
    try:
        ...
        if cur.fetchone()[0] > 0:
            print("ABORT: workflow_templates already exist for tiler."); return
        for suf, key, fp, desc in TEMPLATES:
            cur.execute(
                ...
                (f"7e560000-0000-4000-8000-0000000000{suf}", PROJ_ID, key, fp, desc, "graphic_novel_cell_shaded"),
            )
        for suf, key in ROUTES:
            cur.execute(
                'INSERT INTO workflow_routes (id,"projectId","routeKey","createdAt") VALUES (%s,%s,%s, now())',
                (f"7e560000-0000-4000-8000-0000000000{suf}", PROJ_ID, key),
            )
        conn.commit()
        print("OK seeded workflows for tiler:")
