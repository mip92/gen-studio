...
TEMPLATES=[("b1","char_ip_flux_comic","layoff/comfy/scene_single_character_flux_comic_api.json"),
           ("b2","environment_flux_comic","layoff/comfy/scene_environment_flux_comic_api.json")]
ROUTES=[("b3","layoff_character_ip"),("b4","layoff_environment")]
...
def main():
    ...
    for sub in ("comfy","reference","tts","shots","scenes","bgm"): os.makedirs(os.path.join(ROOT,"data","layoff",sub),exist_ok=True)
    src=os.path.join(ROOT,"data","_templates","comfy"); dst=os.path.join(ROOT,"data","layoff","comfy"); c=0
    for fn in os.listdir(src):
        if fn.endswith(".json"): shutil.copyfile(os.path.join(src,fn),os.path.join(dst,fn)); c+=1
    cx.commit()
    print("OK layoff scenes=%d chars=%d profiles=%d locs=%d comfy=%d"%(len(SCENES),len(CHARS),len(PROFILES),len(LOCS),c))
    cur.close(); cx.close()
