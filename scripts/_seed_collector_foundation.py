...
TEMPLATES=[("b1","char_ip_graphic_novel","collector/comfy/scene_single_character_graphic_novel_api.json"),
           ("b2","environment_graphic_novel","collector/comfy/scene_environment_graphic_novel_api.json")]
ROUTES=[("b3","collector_character_ip"),("b4","collector_environment")]
...
def main():
    ...
    for sub in ("comfy","reference","tts","shots","scenes","bgm"): os.makedirs(os.path.join(ROOT,"data","collector",sub),exist_ok=True)
    src=os.path.join(ROOT,"data","surrogate","comfy"); dst=os.path.join(ROOT,"data","collector","comfy"); c=0
    for fn in os.listdir(src):
        if fn.endswith(".json"): shutil.copyfile(os.path.join(src,fn),os.path.join(dst,fn)); c+=1
    cx.commit()
    print("OK collector scenes=%d chars=%d profiles=%d locs=%d comfy=%d"%(len(SCENES),len(CHARS),len(PROFILES),len(LOCS),c))
    cur.close(); cx.close()
