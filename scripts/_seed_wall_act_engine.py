# -*- coding: utf-8 -*-
"""Shared shot engine for project `wall` (ТЫ — нимфоманка / GDR, Berlin Wall).
Eldritch dark-noir cell-shaded style. Multi-age heroine (c1 with YOUNG/MID/OLD profiles).
Sanitizer strips content negations from positive; per-shot motion fields prevent Wan
spawning anime figures. NON-EXPLICIT: anonymous men are faceless/from-behind silhouettes.
...
CHARS = {
 ...
 "MOTHER":("c2","d4"), "FATHER":("c3","d5"), "ANDREAS":("c4","d6"), "RIVAL_YOUNG":("c5","d7"),
}
def _cid(code): return f"7e580000-0000-4000-8000-0000000000{CHARS[code][0]}"
def _pid(code): return f"7e580000-0000-4000-8000-0000000000{CHARS[code][1]}"

LOC = {k: f"7e580000-0000-4000-8000-000000000{v}" for k, v in {
 "old_flat":"f01","childhood_flat":"f02","courtyard":"f03","the_wall":"f04","checkpoint":"f05",
 "school":"f06","factory":"f07","dormitory":"f08","kneipe":"f09","disco":"f0a","her_room":"f0b",
 "andreas_flat":"f0c","street_east":"f0d","stasi_office":"f0e","clinic":"f0f","wall_fall":"f10",
 "street_west":"f11","cafe_west":"f12","east_side_gallery":"f13","park":"f14",
}.items()}

STYLE = ("dark cinematic graphic novel illustration, gritty noir comic book panel, Eldritch Comics style, "
         "heavy black ink with deep shadows and high-contrast chiaroscuro, cell-shaded muted desaturated cold "
         "palette, gritty cross-hatching, 16:9 cinematic composition, no photorealism, no 3D render, no plastic skin, "
         "moody painterly comic-book aesthetic")
TECH = ("heavy black ink outline with deep noir shadows and high-contrast chiaroscuro, muted desaturated cold "
        "cell-shaded color blocks with gritty cross-hatching")

AMBIENT_MOTION = ("only ambient environmental motion, slow drifting smoke and shifting cold light, faint curtain sway, "
                  "an empty scene with no people and no figures, the noir illustration softly coming to life, hand-drawn animation cadence")
...
ENV_MNEG  = ("people, person, man, woman, human figure, figures, characters, crowd, silhouettes of people, "
             "anime character, anime girl, manga character, extra humans, nudity, sexual content, explicit, "
...
CHAR_MNEG = ("extra people, additional figures, duplicate person, twin, clone, background crowd, anime character, "
             "anime girl, manga character, extra limbs, deformed face, nudity, sexual content, explicit, "
             "photoreal, photograph, 3D render, plastic skin, flicker")

def seed_act(scene_order, palette, mood, render_mode, label, SHOTS, id_region='0'):
    scene_id = f"7e580000-0000-4000-8000-000000000e{scene_order:02x}"
    ...
    try:
        for i, (code, ch, loc, subj, st, ang, mv, tod, broll, iconic, narr) in enumerate(SHOTS, start=1):
            ...
            sid = f"7e580000-0000-4000-8000-{scene_order:02x}{id_region}0000000{i:02x}"
            p = positive(subj, st, ang, mv)
            is_dual = isinstance(ch, (tuple, list))
            codes = list(ch) if is_dual else ([ch] if ch else [])
            route = "wall_character_ip" if codes else "wall_environment"
            ...
            for j, cc in enumerate(codes):
                ...
                cur.execute('INSERT INTO shot_participants (id,"shotId","characterId",label,"profileId") VALUES (%s,%s,%s,%s,%s)',
                    (f"7e580000-0000-4000-8000-{scene_order:02x}90000000{(i+j*0x80):02x}", sid, _cid(cc), lbl, _pid(cc)))
            seeded += 1
        conn.commit()
