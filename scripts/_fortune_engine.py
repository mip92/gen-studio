# -*- coding: utf-8 -*-
"""Shot engine for fortune (гадалка). One-shot creation helper; deleted after verify.
Mirrors cat_lady promptFields shape. NEON is the signature — bake it into subjects/palettes.
Device: ambiguous gift — most shots show cold-reading craft; a few uncanny hits unexplained.
seed_act(scene_key, palette, shots, render_mode='static')
  shots tuple: (code, ch, loc, subj, st, ang, mv, tod, broll, iconic, narr)
"""
import json, psycopg2
PFX  = "7e5d0000-0000-4000-8000-"
...
ENV_MOT  = ("only ambient environmental motion, buzzing flickering neon glow, drifting candle smoke and faint haze, "
    "gently shifting colored light and shadow, an empty scene with no people and no figures, the cell-shaded "
    "illustration softly coming to life, hand-drawn animation cadence")
ENV_MNEG = ("people, person, man, woman, human figure, figures, characters, crowd, silhouettes of people, anime character, "
    ...
    "plastic skin, deformed, flicker artifacts, warping")
CHAR_MOT = ("subtle natural motion of only the figure already present in the frame, small breathing and quiet "
    "micro-movements continuing the moment, buzzing neon and drifting candle smoke around them, no new people entering, "
    "no extra figures appearing, the cell-shaded illustration in motion, hand-drawn animation cadence")
CHAR_MNEG= ("extra people, additional figures, duplicate person, twin, clone, background crowd, anime character, anime girl, "
    "manga character, extra limbs, deformed face, photoreal, photograph, 3D render, plastic skin, warping")
...
_NEG = ("no people","no person","no figures","no faces","no one","without people","empty of people","nobody")

def _camera(st, ang, mv): return "%s, %s angle, %s camera, 16:9" % (st, ANG[ang], MOVES[mv])
def _san(subj):
    low=subj.lower()
    for n in _NEG:
        if n in low: raise SystemExit("NEGATION in positive: %r in %r"%(n,subj))
    return subj
...
def seed_act(scene_key, palette, shots, render_mode="static", id_region="0"):
    ...
    for idx,(code,ch,loc,subj,st,ang,mv,tod,broll,iconic,narr) in enumerate(shots):
        ...
        route="fortune_character_ip" if is_char else "fortune_environment"
