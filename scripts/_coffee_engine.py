# -*- coding: utf-8 -*-
"""Shot engine for coffee («ТЫ — Хозяйка кофейни»). Clone of _collector_engine with coffee OBJECTS/routes.
Warm storybook cell-shaded (Eldritch_Classic_Comics). Track B bright, f5 female.
  - UNIQUE per-shot motionPrompt derived from subj (§5.3b) + guard tail
  - OBJECTS prop-hero: ch="OBJ:<key>" -> env route, loc=None, anchor + DOF (§17.3)
  - every shot carries VO; env forced renderMode='static'; VO window 12..20 reported
...
ROUTE_C="coffee_character_ip"; ROUTE_E="coffee_environment"
...
OBJECTS={
 "cezve":"a small copper cezve pot with a bent wooden handle, its hammered sides polished bright by decades of hands",
 "cinnamon":"a squat glass jar of ground cinnamon with a hand-written paper label and a tiny wooden scoop",
 "cup":"a bright orange takeaway coffee cup with a simple cinnamon-stick emblem printed on the sleeve",
 "box_cafe":"a child's cardboard-box play cafe with a cut-out serving window and a crooked marker-drawn menu",
 "notebook":"a fat dog-eared notebook bursting with pasted clippings and pencil sketches, a rubber band holding it shut",
 "apron":"a bright yellow canvas apron with flour and cinnamon dust worked into its creases",
}
...
ENV_GUARD=(", with only ambient environmental motion, drifting steam and dust motes in warm sunlight, gently shifting light and shadow, "
