# -*- coding: utf-8 -*-
"""Пред-сидовый аудит таблицы кадров towtruck. Ничего не пишет, только считает.

    python scripts/_check_towtruck_shots.py
"""
import io
import json
import os
import re
import sys

sys.stdout.reconfigure(encoding="utf-8")
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
from seed_towtruck_shots import SHOTS  # noqa: E402

HERE = os.path.dirname(os.path.abspath(__file__))
TPL = json.load(io.open(os.path.join(HERE, "comic_page_templates.json"), encoding="utf-8"))
TPL = {t["id"]: [s["shape"] for s in sorted(t["slots"], key=lambda s: s["order"])]
       for t in TPL["templates"]}

PAGES = ["duo_wide", "tall_page_left", "land_sq_squares", "narrows3_land2",
         "talls_flank_squares", "tall_page_right", "duo_wide", "bands_squares_mid",
         "land2_narrows3", "hero_land_squares", "duo_wide", "squares_hero_land",
         "wide_top_trio", "bands_squares_mid", "tall_page_right", "land_sq_squares",
         "land2_narrows3", "trio_wide_bottom", "narrows3_land2", "tall_page_left",
         "bands_squares_mid", "squares_hero_land", "land_sq_squares", "wide_top_trio",
         "land2_narrows3", "talls_flank_squares", "narrows3_land2", "sq_land_squares",
         "bands_squares_mid", "tall_page_right", "land_sq_squares", "wide_narrows4",
         "tall_page_left", "talls_flank_squares", "hero_land_squares", "duo_wide",
         "land2_narrows3", "squares_hero_land", "narrows3_land2", "land_sq_squares",
         "tall_page_right", "bands_squares_mid", "narrows3_land2", "duo_wide"]

ACT_TARGET = {"A1": 9, "A2": 16, "A3": 18, "A4": 18, "A5": 18, "A6": 18,
              "A7": 20, "A8": 18, "A9": 20, "A10": 18, "FIN": 17}

OPEN_OK = {
    "wide": ("wide shot", "extreme wide establishing shot"),
    "landscape": None,
    "square": ("close-up", "extreme close-up", "medium close-up", "medium shot"),
    "tall": ("full-length shot", "medium shot", "low-angle full-length shot"),
    "narrow": ("full-length shot", "medium shot", "medium close-up", "close-up"),
    "tall_page": ("full-length shot", "low-angle full-length shot"),
}

# вводное слово ↔ колонка shotType (§5A.3b: разъехались — врём аудиту масштабов)
OPEN_TYPE = {
    "extreme wide establishing shot": {"EWS"},
    "wide shot": {"WS"},
    "full-length shot": {"FS"},
    "low-angle full-length shot": {"FS"},
    "medium shot": {"MS"},
    "medium close-up": {"MCU"},
    "close-up": {"CU"},
    "extreme close-up": {"ECU"},
}

GROUP = {"EWS": "W", "WS": "W",
         "MS": "M", "MCU": "M", "OTS": "M",
         "CU": "C", "ECU": "C", "FS": "C", "INSERT": "C", "POV": "C", "BACK": "C"}

LOCS = {"clinic_entrance", "courtyard_old", "dispatch_room", "flat_kitchen",
        "flyover_night", "impound_lot", "narrow_street", "pharmacy",
        "ramp_incline", "tow_cab", "watch_hut"}
PROPS = {"FATHER_SEDAN", "TOWTRUCK", "TOY_TRUCK", "WINCH"}
PROFS = {"ARTEM_KID", "ARTEM_TEEN", "KOSTYA_MID", "LENA_MID", "RITA_MID",
         "VIKTOR_KID", "VIKTOR_LATE", "VIKTOR_MID"}
MOVES = {"static", "locked_off", "push_in", "pull_out", "track", "track_lateral",
         "pan", "pan_left", "pan_right", "tilt_up", "tilt_down", "handheld",
         "window_pov", "arc_left", "arc_right"}
ANGLES = {"eye", "low", "high", "top", "top_down", "over", "pov", "dutch"}

# «стоит» = «находится» ловушкой не является (замерено на корпусе, 103 попадания
# прочитаны верно); запрещена только ЦЕНОВАЯ форма «стоил/стоило/стоимость».
VO_BAN = [r"\bсорок\w*", r"\bстоил\w*", r"\bстоимост\w*", r"понимаешь,? что", r"ловишь себя",
          r", и это\b", r"\+"]
POS_BAN = [r"\bno\b", r"\bnot\b", r"\bwithout\b", r"\bnever\b", r"\bempty of\b",
           r"the camera", r"masterpiece", r"best quality", r"photoreal", r"anime",
           r"cell-shaded", r"palette of"]
MOT_BAN = [r"\bsays?\b", r"\bsaid\b", r"\bspeak\w*", r"\btalk\w*", r"\bvoices?\b",
           r"\bchatter\b", r"\bshout\w*", r"\bargu\w*", r"\bon the phone\b",
           r"\bthen we cut\b", r"\bnext shot\b", r"\bmeanwhile\b", r"\bmusic\b"]
LOCKS = ("the place stays deserted", "the same single figure",
         "the same two figures", "the background figures holding",
         "the same hands already in frame")

NAMES = {"Артём", "Артёму", "Артёма", "Лена", "Лену", "Лене", "Костя", "Костю",
         "Косте", "Костина", "Рита", "Риту", "Рите", "Виктор"}

errs, warns = [], []


def E(code, msg):
    errs.append(f"{code}: {msg}")


def W(code, msg):
    warns.append(f"{code}: {msg}")


seen = set()
by_act = {}
for row in SHOTS:
    (code, page, slot, shape, stype, angle, move, loc, prof, prop,
     ru, pos, mot, flags) = row
    act = code.split("_")[0]
    by_act.setdefault(act, []).append(row)
    if code in seen:
        E(code, "дубль shotCode")
    seen.add(code)

    # --- вёрстка ---
    if not (0 <= page < len(PAGES)):
        E(code, f"страница {page} вне книги")
    else:
        shapes = TPL[PAGES[page]]
        if not (0 <= slot < len(shapes)):
            E(code, f"слот {slot} вне шаблона {PAGES[page]} ({len(shapes)} слотов)")
        elif shapes[slot] != shape:
            E(code, f"форма {shape} != {shapes[slot]} (стр {page} {PAGES[page]} слот {slot})")

    # --- перечисления колонок ---
    if move not in MOVES:
        E(code, f"cameraMove '{move}' нет в карте движка")
    if angle not in ANGLES:
        E(code, f"cameraAngle '{angle}' незнакомый")
    if stype not in GROUP:
        E(code, f"shotType '{stype}' незнакомый")
    if loc is not None and loc not in LOCS:
        E(code, f"локация '{loc}' не существует")
    if prop is not None and prop not in PROPS:
        E(code, f"props '{prop}' не существует")
    if prop is not None and loc is not None:
        E(code, "кадр-предмет обязан идти без локации (§17.3)")
    profs = () if prof is None else ((prof,) if isinstance(prof, str) else tuple(prof))
    for p in profs:
        if p not in PROFS:
            E(code, f"профиль '{p}' не существует")
    if len(profs) > 3:
        E(code, "больше трёх референсов Qwen не берёт")

    # --- VO ---
    if not re.search(r"[.!?]$", ru):
        E(code, "VO без терминального знака (f5 даст придыхание)")
    words = len(ru.split())
    if words < 9:
        E(code, f"VO {words} слов — короче четырёх секунд")
    if words > 24:
        W(code, f"VO {words} слов — длинновато")
    for pat in VO_BAN:
        m = re.search(pat, ru, re.I)
        if m:
            E(code, f"VO запрещённое '{m.group(0)}'")
    if re.search(r"\d", ru):
        E(code, "VO содержит цифру — числа только словами")
    if re.search(r"[A-Za-z]", ru):
        E(code, "VO содержит латиницу")
    first = ru.split()[0].strip("«»,.")
    if first[:1].isupper() and first not in NAMES:
        E(code, f"VO начинается с заглавной '{first}'")
    if ru.count(".") + ru.count("!") + ru.count("?") > 1:
        E(code, "VO больше одного предложения (f5 деградирует)")

    # --- позитив ---
    pw = len(pos.split())
    if pw > 55:
        E(code, f"позитив {pw} слов при бюджете 55 — хвост обрежется")
    if pw < 25:
        W(code, f"позитив {pw} слов — тонковато")
    for pat in POS_BAN:
        m = re.search(pat, pos, re.I)
        if m:
            E(code, f"позитив запрещённое '{m.group(0)}'")
    allowed = OPEN_OK[shape]
    head = pos.split(",")[0].strip().lower()
    opener = None
    for o in sorted(OPEN_TYPE, key=len, reverse=True):
        if head.startswith(o):
            opener = o
            break
    if opener is None:
        E(code, f"вводное слово не распознано: '{head}'")
    else:
        if allowed is not None and opener not in allowed:
            E(code, f"вводное '{opener}' спорит с формой {shape}")
        if stype not in OPEN_TYPE[opener]:
            E(code, f"вводное '{opener}' против shotType '{stype}'")

    # --- моушен ---
    if "no music" not in mot:
        E(code, "моушен без 'no music' (§2.4)")
    if not any(l in mot for l in LOCKS):
        E(code, "моушен без позитивного замка")
    if profs and "the place stays deserted" in mot:
        E(code, "замок пустоты на кадре с участником")
    # §4c(b): кадр, где субъект — руки, не может нести замок безлюдного места.
    LIMB = r"\b(hand|hands|palm|palms|finger|fingers|thumb|forearm|forearms|glove|" \
           r"gloved|knee|knuckles|arms|cuff|elbows)\b"
    # «hand-written», «hand lever», «hand-welded» — это НЕ конечность в кадре.
    pos_limb = re.sub(r"\bhand[- ](?:written|welded|lever|painted|drawn)\b", "", pos, flags=re.I)
    if (not profs and re.search(LIMB, pos_limb, re.I)
            and "the place stays deserted" in mot):
        E(code, "замок пустоты на кадре, где субъект — руки (§4c(b))")
    for pat in MOT_BAN:
        for m in re.finditer(pat, mot, re.I):
            if m.group(0).lower() == "music" and "no music" in mot:
                continue
            E(code, f"моушен запрещённое '{m.group(0)}'")
    mw = len(mot.split())
    if not (30 <= mw <= 90):
        W(code, f"моушен {mw} слов (норма 40–80 с замком)")

# --- количество кадров по актам ---
for act, want in ACT_TARGET.items():
    got = len(by_act.get(act, []))
    if got != want:
        E(act, f"кадров {got}, по плану {want}")
if len(SHOTS) != 190:
    E("ALL", f"всего кадров {len(SHOTS)}, ожидалось 190")

# --- покрытие книги: каждый слот каждой страницы занят ровно раз ---
used = {}
for row in SHOTS:
    used.setdefault((row[1], row[2]), []).append(row[0])
for pi, tid in enumerate(PAGES):
    for si in range(len(TPL[tid])):
        who = used.get((pi, si), [])
        if len(who) != 1:
            E(f"page{pi}/slot{si}", f"занят {len(who)} раз ({who})")

# --- чередование масштабов внутри акта (§5.1) ---
for act, rows in by_act.items():
    g = [GROUP[r[4]] for r in rows]
    for i in range(2, len(g)):
        if g[i] == g[i - 1] == g[i - 2]:
            E(rows[i][0], f"третий кадр группы {g[i]} подряд ({rows[i-2][0]}, {rows[i-1][0]})")

# --- швы между актами ---
order = list(ACT_TARGET)
flat = [r for a in order for r in by_act[a]]
g = [GROUP[r[4]] for r in flat]
for i in range(2, len(g)):
    if g[i] == g[i - 1] == g[i - 2]:
        a = {flat[i][0].split("_")[0], flat[i - 1][0].split("_")[0]}
        if len(a) > 1:
            E(flat[i][0], f"шов актов: третий кадр группы {g[i]} подряд")

# --- лексика и самоповторы ---
vo = [r[10] for r in flat]
n_vpervye = sum(1 for t in vo if "впервые" in t.lower())
if n_vpervye > 3:
    E("ALL", f"«впервые» {n_vpervye} раз (норма 3)")
for phrase in ("потому что", ", и это", "ты понимаешь"):
    n = sum(t.lower().count(phrase) for t in vo)
    if phrase == "потому что" and n > 2:
        W("ALL", f"«потому что» {n} раз (норма 2)")
if len(set(vo)) != len(vo):
    E("ALL", "дословные дубли строк VO")
poss = [r[11] for r in flat]
if len(set(poss)) != len(poss):
    E("ALL", "дословные дубли позитивов")
mots = [r[12] for r in flat]
if len(set(mots)) != len(mots):
    E("ALL", "дословные дубли моушен-промптов (§5.3b: уникальный на кадр)")

# трёхграммы VO
from collections import Counter  # noqa: E402
tri = Counter()
for t in vo:
    w = re.findall(r"\w+", t.lower())
    for i in range(len(w) - 2):
        tri[" ".join(w[i:i + 3])] += 1
for k, v in tri.most_common(12):
    if v >= 3:
        W("ALL", f"трёхграмма ×{v}: «{k}»")

# --- метраж ---
tot = sum(len(t.split()) for t in vo)
mins = tot / 140 + len(vo) * 0.5 / 60
print(f"кадров: {len(SHOTS)}   слов VO: {tot}   ≈ {mins:.1f} мин")
no_people = sum(1 for r in flat if "the place stays deserted" in r[12])
unanchored = sum(1 for r in flat if r[8] is None and "the place stays deserted" not in r[12])
print(f"кадров без людей: {no_people} ({no_people * 100 // len(flat)}%)"
      f"   людей без якоря: {unanchored}"
      f"   кадров-предметов: {sum(1 for r in flat if r[9])}"
      f"   иконических: {sum(1 for r in flat if 'i' in r[13])}")
gc = Counter(GROUP[r[4]] for r in flat)
print("масштабы:", dict(gc))
print("формы:", dict(Counter(r[3] for r in flat)))
print()
if errs:
    print(f"ОШИБКИ ({len(errs)}):")
    for e in errs:
        print("  ✗", e)
else:
    print("ошибок нет")
if warns:
    print(f"\nпредупреждения ({len(warns)}):")
    for w in warns:
        print("  ·", w)
sys.exit(1 if errs else 0)
