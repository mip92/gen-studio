# -*- coding: utf-8 -*-
"""Shot engine for cloakroom («ТЫ — Гардеробщик»). Mirrors _mascot_engine.
Track B, 2nd person, f5. Theatre interior — ENV motion tuned for dim velvet/dust (not heat-haze).
...
seed_act(scene_key, palette, shots, render_mode='static', id_region='0')
  ...
  ch: None=env | profileCode (CLOAK_OLD / COATMAN_MID / GADALKA_BASE / LIGA_BASE ...)
"""
...
ENV_MOT=("only ambient environmental motion, slow drifting dust motes in a shaft of light, gently shifting dim light and shadow, "
 "a faint draught stirring velvet and hanging coats, an empty still scene with no people and no figures, "
 "the cell-shaded illustration barely coming to life, hand-drawn animation cadence")
...
def seed_act(scene_key, palette, shots, render_mode="static", id_region="0"):
    ...
    for idx,(code,ch,loc,subj,st,ang,mv,tod,broll,iconic,narr) in enumerate(shots):
        ...
        route="cloakroom_character_ip" if is_char else "cloakroom_environment"
