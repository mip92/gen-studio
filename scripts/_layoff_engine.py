# -*- coding: utf-8 -*-
"""Shot engine for layoff («ТЫ — Уволенный»). Mirrors _cloakroom_engine, STYLE=flux modern-comic.
Track B, 2nd person, f5. §5.3a: env (no participant) shots -> renderMode='static' always.
seed_act(scene_key, palette, shots, render_mode='static', id_region='0')
  ...
  ch: None=env | profileCode (GEROY_MAIN / ELYA_BASE / ...)
"""
import json, psycopg2
PFX="7e610000-0000-4000-8000-"; PROJ=PFX+"000000000001"
ROUTE_C="layoff_character_ip"; ROUTE_E="layoff_environment"
...
ENV_MOT=("only ambient environmental motion, slow drifting dust motes in the light, a faint flicker of fluorescent light and shifting shadow, "
 "an empty still scene with no people and no figures, the cell-shaded illustration barely coming to life, hand-drawn animation cadence")
...
def seed_act(scene_key, palette, shots, render_mode="static", id_region="0"):
    ...
    print("  %-14s shots=%2d scale3run=%s short=%s nodot=%s"%(scene_key,len(shots),(",".join(runs) if runs else "0"),(",".join("%s:%d"%(c,w) for c,w in short) if short else "0"),(",".join(nodot) if nodot else "0")))
