# -*- coding: utf-8 -*-
"""Shot engine for collector («ТЫ — Коллектор»). cell_shaded (Eldritch_Classic_Comics), warm storybook comic.
Track B, 2nd person, f5 (male). Upgrades over surrogate engine:
  - UNIQUE per-shot motionPrompt derived from subj (§5.3b) + anti-hallucination guard tail
  - OBJECTS prop-hero anchors: ch="OBJ:<key>" -> env route, loc=None, anchor + DOF composed in (§17.3)
  - every shot must carry VO (feedback-broll-must-carry-vo) — empty narr reported loudly
  - env shots forced renderMode='static' (§5.3a); VO window 12..20 words reported
...
ROUTE_C="collector_character_ip"; ROUTE_E="collector_environment"
...
OBJECTS={
 "lighter":"a worn brass petrol lighter, its hinged lid scratched and the lacquer rubbed to bare metal at the corners",
 "headset":"a black call-center headset with a thin microphone boom and a cracked worn foam ear pad",
 "script_card":"a laminated call-script card dog-eared at one corner, dense small print with one line highlighted in yellow marker",
 "sticker":"a garish yellow debt-notice sticker with a thick black exclamation mark pasted across a door seam",
 "peephole":"a round brass door peephole set in a dark padded apartment door",
}
...
ENV_GUARD=(", with only ambient environmental motion, slow drifting dust in soft light, gently shifting light and shadow, "
