...
export class ImageValidationService {
  ...
  async enqueue(shotId: string, expectedPrompt: string | null) {
    ...
    const job = await this.db.imageValidationJob.create({
      data: {
        shotId,
        status:         'pending',
    ...
    await this.ledger.enqueue('validation', job.id, { paramsSnapshot: { candidates: unscored } });
    return job;
  }
  ...
  async run(jobId: string): Promise<void> {
    ...
    try {
      ...
      await this.ledger.close('validation', jobId, { status: 'completed', outputFilename: winner?.filename ?? null });
      ...
      await this.ledger.close('validation', jobId, { status: 'failed', errorMessage: String(e?.message ?? e) });
    }
  }
